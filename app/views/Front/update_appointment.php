...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    $appointmentController->updateAppointment($id, $patient_id, $doctor_id, $appointment_date);
...
<body>
    ...
    <div class="container mt-5">
        <h2 class="my-4">Modifier Rendez-vous</h2>
        <form action="update_appointment.php?id=<?php echo $id; ?>" method="POST">
            <div class="mb-3">
                <label for="patient_id" class="form-label">Patient ID:</label>
                <input type="text" id="patient_id" name="patient_id" class="form-control" value="<?php echo htmlspecialchars($appointment['patient_id']); ?>" required>
            </div>
            <div class="mb-3">
                <label for="doctor_id" class="form-label">Doctor ID:</label>
                <input type="text" id="doctor_id" name="doctor_id" class="form-control" value="<?php echo htmlspecialchars($appointment['doctor_id']); ?>" required>
            </div>
            <div class="mb-3">
                <label for="appointment_date" class="form-label">Date et Heure:</label>
                <input type="datetime-local" id="appointment_date" name="appointment_date" class="form-control" value="<?php echo htmlspecialchars(date('Y-m-d\TH:i', strtotime($appointment['appointment_date']))); ?>" required>
            </div>
            <input type="submit" value="Mettre à jour" class="btn btn-primary">
        </form>

        <!-- Bouton Retour -->
        <a href="admin_appointments.php" class="btn btn-secondary mt-3">Retour</a>
    </div>
    <!-- Main Content End -->
